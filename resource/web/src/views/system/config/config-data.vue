<template>
  <div>
    <!-- 数据表格 -->
    <ele-pro-table @selection-change="handleSelectionChange" ref="table" :datasource="url" :columns="columns"
                   :where="where" height="calc(100vh - 261px)" tool-class="ele-toolbar-form">
      <!-- 表头工具栏 -->
      <template slot="toolbar">
        <el-form :model="where" class="ele-form-search" size="small" @keyup.enter.native="reload"
                 @submit.native.prevent>
          <el-row :gutter="10">
            <el-col :md="6">
              <el-form-item>
                <el-input v-model="where.title" placeholder="请输入配置项标题" clearable size="small"/>
              </el-form-item>
            </el-col>
            <el-col :md="6">
              <el-form-item>
                <el-input v-model="where.code" placeholder="请输入配置编码" clearable size="small"/>
              </el-form-item>
            </el-col>
            <el-col :md="10">
              <el-form-item>
                <el-button type="primary" @click="reload" icon="el-icon-search" class="ele-btn-icon" size="small">查询
                </el-button>
                ...
                <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small"
                           v-if="permission.includes('sys:config:dall')">批量删除
                </el-button>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <template slot="action" slot-scope="{row}">
        ...
        <el-popconfirm title="确定要删除此配置项吗？" @confirm="remove(row)" class="ele-action">
    ...
    <!-- 编辑弹窗 -->
    <config-data-edit :visible.sync="showEdit" :data="current" :config-id="configId" @done="reload"/>
  </div>
</template>
    ...
    methods: {
      ...
      remove(row) {
        if (row) {
          ...
          this.$http.post('/config_data/delete', {
            id: row.id
          }).then(res => {
            loading.close();
            if (res.data.code === 0) {
          ...
          const ids = this.choose.map(d => d.id)
          this.$confirm('确定要删除选中的配置项吗?', '提示', {
            type: 'warning'
          }).then(() => {
            const loading = this.$loading({
              lock: true
            })
            this.$http.post('/config_data/batchdelete', {
              ids
            }).then(res => {
              loading.close()
              if (res.data.code === 0) {
      ...
      editStatus(row) {
        ...
        this.$http.post('/config_data/status', params).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({
              type: "success",
